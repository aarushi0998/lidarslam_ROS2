...
//#include "../livox_ros_driver2/CustomMsg.h"  // Assuming message file is CustomMsg.msg
//#include "CustomPoint.hpp"  // Assuming message file is CustomMsg.msg
//#include "../livox_ros_driver2/msg/CustomMsg.hpp"  // Assuming your package builds to 'msg.hpp'
#include "/home/aria/lidarslam_ws/src/LIO-SAM/livox_ros_driver2/CustomMsg.hpp"
#include <memory>
...
rclcpp::Subscription<livox_ros_driver2::CustomMsg>::SharedPtr subLaserCloud;
...
// std::shared_ptr<rclcpp::Subscription<livox_ros_driver2::CustomMsg>> subLaserCloud;
// std::shared_ptr<rclcpp::CallbackGroup> callbackGroupLidar;
// std::shared_ptr<rclcpp::Publisher<sensor_msgs::msg::PointCloud2>> pubLaserCloud;

// std::shared_ptr<rclcpp::Publisher<sensor_msgs::msg::PointCloud2>> pubExtractedCloud;
// std::shared_ptr<rclcpp::Publisher<lio_sam::msg::CloudInfo>> pubLaserCloudInfo;

// std::shared_ptr<rclcpp::Subscription<sensor_msgs::msg::Imu>> subImu;
// std::shared_ptr<rclcpp::CallbackGroup> callbackGroupImu;
// std::deque<sensor_msgs::msg::Imu> imuQueue;  // No change needed here

// std::shared_ptr<rclcpp::Subscription<nav_msgs::msg::Odometry>> subOdom;
// std::shared_ptr<rclcpp::CallbackGroup> callbackGroupOdom;
...
public:
           ...
           ParamServer("lio_sam_imageProjection", options), deskewFlag(0)
   {
       ...
       subOdom = create_subscription<nav_msgs::msg::Odometry>(
           ...
           odomOpt);
       subLaserCloud = create_subscription<livox_ros_driver2::CustomMsg>(
   ...
   void cloudHandler(const std::shared_ptr<livox_ros_driver2::CustomMsg> laserCloudMsg)
   ...
   //const std::shared_ptr<livox_ros_driver2::CustomMsg>& Msg
   /*the below is the orginal from the ROS1 LIO-SAM-MID360. Might have to change it to the ROS2 version if this doesn't work. The ROS2 version might loook like:
   
     cloud->clear();
     cloud->reserve(msg->point_num);
      PointXYZIRT point;
       cloud->header.frame_id = msg->header.frame_id;
     cloud->header.stamp = rclcpp::Time(msg->header.stamp.toNSec() / 1000).nanoseconds();  // Convert timestamp to ROS 2 format
 ...
 */
       ...
       //cloud->header.stamp = rclcpp::Time(Msg->header.stamp.toNSec() / 1000).nanoseconds();  // Convert timestamp to ROS 2 format
       //cloud->header.stamp = rclcpp::Time(Msg->header.stamp.sec * 1000000000 + Msg->header.stamp.nanosec);
   ...
   bool cachePointCloud(const std::shared_ptr<livox_ros_driver2::CustomMsg> laserCloudMsg)
   {
       //const std::shared_ptr<livox_ros_driver2::CustomMsg>& laserCloudMsg
       // cache point cloud
